...
| ```pulse_interval_secs```    | Integer  |  Indicates a [coordinated update](client-commands.md#user-content-coordinated-job-updates-beta). If no pulses received within provided interval the update gets blocked. Beta-updater only. Will fail on submission when used with client updater. (Default: None)
