...
### Job Objects
  ...
  ```production``` | Boolean |  Whether or not this is a production task (Default: False). Production jobs may preempt any non-production job, and may only be preempted by production jobs in the same role and of higher priority. To run jobs without a dedicated constraint at this level, the job role must have the appropriate quota. See [this document](resource-isolation.md#resource-quota) for more details.
