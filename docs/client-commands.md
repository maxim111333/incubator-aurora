...
Some Aurora services may benefit from having more control over the asynchronous scheduler updater
by explicitly acknowledging ("heartbeating") the job update progress. This may be helpful for
mission-critical service updates where explicit job health monitoring is vital during the entire
job update lifecycle. Such job updates would rely on an external service (or a custom client)
periodically pulsing an active coordinated job update via a
